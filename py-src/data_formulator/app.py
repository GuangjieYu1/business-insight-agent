...
def _is_backend_oidc_mode() -> bool:
    """Return True only when backend OIDC is explicitly configured.

    This helper avoids importing the OIDC provider module during startup,
    because that module pulls in ``jwt``/``cryptography`` at import time.
    """
...
def _register_blueprints():
    ...
    # `disable_custom_models` only blocks user-managed model endpoints in the
    # UI. The core `/api/agent/*` routes are still required for app startup
    # and for server-configured models, so keep the blueprint registered.
    ...
    # Demo streaming and BIA extension routes pull in optional data source
    # dependencies. Keep them out of the local safe boot path so the app can
    # start even when those native wheels are not usable in this runtime.
    if app.config['CLI_ARGS']['product_mode'] == ProductMode.BUSINESS_INSIGHT.value:
    ...
    # Register auth token management routes only when auth is configured.
    # Anonymous container boots do not need the OIDC gateway stack, and
    # importing it pulls in cryptography/jwt native wheels that are not
    # always usable in this runtime.
    if provider is not None or _is_backend_oidc_mode():
    ...
    # Auto-register data loaders only when the deployment wants them.
    # In local no-connector mode we skip the whole loader registry import,
    # because that module eagerly scans all built-in loader backends.
    if app.config['CLI_ARGS'].get('disable_data_connectors'):
        print("  External data connectors disabled (DISABLE_DATA_CONNECTORS=true) — startup skips loader registry", flush=True)
    else:
